...
(
    ...
    `browseType` enum ('flow','single','double') NULL DEFAULT 'single' comment '浏览方式',
    `page`       int(11)                         NULL DEFAULT NULL comment '页码',
    `pageImage`  varchar(255)                    NULL DEFAULT NULL comment '页码图片',
...
(
    ...
    `chapterName`  varchar(255)                    NULL DEFAULT NULL COMMENT '章节名称',
    `chapterPath`  varchar(255)                    NULL DEFAULT NULL COMMENT '章节路径',
    `chapterType`  varchar(255)                    NULL DEFAULT NULL COMMENT '文件类型',
    `browseType`   enum ('flow','single','double') NULL DEFAULT 'flow' COMMENT '浏览方式',
    `chapterCover` varchar(255)                    NULL DEFAULT NULL COMMENT '章节封面',
...
(
    ...
    `compressType`   enum ('zip','rar','pdf','image','7z')            NULL DEFAULT NULL COMMENT '转换类型',
    `compressPath`   varchar(255)                                     NULL DEFAULT NULL COMMENT '转换路径',
    `compressStatus` enum ('uncompressed','compressing','compressed') NULL DEFAULT NULL COMMENT '转换状态',
...
(
    ...
    `mediaId`     int(11)          NULL DEFAULT NULL COMMENT '媒体库id',
    `mangaId`     int(11)          NULL DEFAULT NULL COMMENT '漫画id',
    `mangaName`   varchar(255)     NULL DEFAULT NULL COMMENT '漫画名称',
    `chapterId`   int(11)          NULL DEFAULT NULL COMMENT '章节id',
    `chapterName` varchar(255)     NULL DEFAULT NULL COMMENT '章节名称',
    `chapterPath` varchar(255)     NULL DEFAULT NULL COMMENT '章节路径',
    `createTime`  datetime(0)      NULL DEFAULT NULL COMMENT '创建时间',
...
(
    ...
    `pathId`       int(11)                         NULL DEFAULT NULL COMMENT '路径id',
    `mangaName`    varchar(255)                    NOT NULL COMMENT '漫画名称',
    `mangaPath`    varchar(255)                    NULL DEFAULT NULL COMMENT '漫画路径',
    `mangaCover`   varchar(255)                    NULL DEFAULT NULL COMMENT '漫画封面',
...
(
    ...
    `mediaName`       varchar(255)                    NOT NULL COMMENT '媒体库名称',
    `mediaType`       int(1)                          NOT NULL COMMENT '媒体库类型 0->漫画 1->单本',
    `mediaCover`      varchar(255)                    NULL DEFAULT NULL COMMENT '媒体库封面',
    `directoryFormat` int(1)                          NULL DEFAULT NULL COMMENT '目录格式 \r\n0 漫画 -> 章节 -> 图片\r\n1 目录 -> 漫画 -> 章节 -> 图片\r\n2 漫画 -> 图片\r\n3 目录 -> 漫画 -> 图片\r\n\r\n23为单本',
    `fileType`        int(1)                          NULL DEFAULT NULL COMMENT '文件类型 0->图片 1->pdf',
    `defaultBrowse`   enum ('flow','single','double') NULL DEFAULT 'flow' COMMENT '默认浏览类型',
    `createTime`      datetime(0)                     NULL DEFAULT NULL COMMENT '创建时间',
    `updateTime`      datetime(0)                     NULL DEFAULT NULL COMMENT '最新修改时间',
    `direction`       int(1)                          NULL DEFAULT 1 COMMENT '翻页方向 0 左到右; 1右到左',
    `removeFirst`     int(1)                          NULL DEFAULT 0 COMMENT '剔除首页 01',
...
-- ----------------------------
INSERT INTO `user`
VALUES (1, 'smanga', 'f7f1fe7186209906a97756ff912bb644', NULL, NULL, NULL);
